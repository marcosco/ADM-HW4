...
# published at the imput conference at least once
...
def my_dijkstra(G, source, target=None):
    ...
    dist[source] = 0
    # All nodes are initiali set as Unvisited
    Q = G.nodes()
    ...
    while Q:
        ...
        minWeight = np.inf
        u = None
        # Select the neighbor with the minimum distance
        for k,v in d.items():
            if k in Q and v["weight"] < minWeight:
                minWeight = v["weight"]
                u = k
        # If a neighbor is selected
        if u:
